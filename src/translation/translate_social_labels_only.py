...
It intentionally does not use recent or aggregate graph state.
"""
...
DEFAULT_OUTPUT_PATH = project_path(Path("data") / "translation_eval" / "translation_social_labels_only.csv")
DEFAULT_PROMPT_PATH = project_path(Path("prompts") / "translate_social_labels_only.txt")
...
class SocialLabelsOnlyResult:
    """Summary information for a social-label-guided translation run."""
...
def load_prompt_template(prompt_path: Path = DEFAULT_PROMPT_PATH) -> str:
    """Load the social-label-guided prompt template."""
    if not prompt_path.exists():
        raise FileNotFoundError(
            f"Prompt file not found: {prompt_path}. "
            "Create prompts/translate_social_labels_only.txt before running this script."
        )
...
def build_social_labels_only_prompt(
    row: pd.Series,
    prompt_template: str | None = None,
    relationship_schema: dict[str, Any] | None = None,
    power_respect_schema: dict[str, Any] | None = None,
) -> str:
    """Build the social-label-guided translation prompt for one row."""
...
def parse_social_labels_only_response(response_text: str) -> tuple[str, str, str]:
    """Parse model output into guidance summary and translation."""
    parsed = extract_json_object(response_text)
    guidance = str(parsed.get("social_guidance_summary", "")).strip()
    translation = str(parsed.get("translation_social_labels_only", "")).strip()
    if not translation:
        raise ValueError("Model response is missing translation_social_labels_only.")
...
def translate_prompt(
    ...
    if dry_run:
        return json.dumps(
            {
                "social_guidance_summary": "[DRY RUN SOCIAL-LABEL GUIDANCE SUMMARY]",
                "translation_social_labels_only": "[DRY RUN SOCIAL-LABEL-GUIDED TRANSLATION]",
            },
            ensure_ascii=False,
        )
...
def get_completed_keys(existing_df: pd.DataFrame) -> set[tuple[str, str, str]]:
    ...
    if existing_df.empty:
        return set()
    required_columns = set(JOIN_KEYS + ["translation_social_labels_only"])
    if not required_columns.issubset(existing_df.columns):
        return set()

    completed_df = existing_df[
        existing_df["translation_social_labels_only"].notna()
        & existing_df["translation_social_labels_only"].astype(str).str.strip().ne("")
    ]
    return {make_row_key(row) for _, row in completed_df.iterrows()}


def translate_social_labels_only_dataset(
    ...
    dry_run: bool = False,
) -> SocialLabelsOnlyResult:
    """Generate social-label-guided translations for the input dataset."""
    load_dotenv()
    ...
    progress_bar = tqdm(
        ...
        desc="Translating social-label-guided",
        unit="row",
        dynamic_ncols=True,
    )
    ...
    for row in progress_bar:
        ...
        prompt = build_social_labels_only_prompt(
            row,
            prompt_template=prompt_template,
            relationship_schema=relationship_schema,
            power_respect_schema=power_respect_schema,
        )
        row_output["translation_prompt_social_labels_only"] = prompt
        row_output["translation_model_social_labels_only"] = model
        row_output["translation_provider_social_labels_only"] = provider
        ...
        try:
            ...
            guidance, translation, raw_response = parse_social_labels_only_response(response_text)
            row_output["llm_social_labels_only_guidance_summary"] = guidance
            row_output["translation_social_labels_only"] = translation
            row_output["translation_raw_response_social_labels_only"] = raw_response
            row_output["translation_status_social_labels_only"] = "success"
            row_output["translation_error_social_labels_only"] = ""
            success_count += 1
        except Exception as error:  # noqa: BLE001 - keep row-level failures in output CSV
            row_output["llm_social_labels_only_guidance_summary"] = ""
            row_output["translation_social_labels_only"] = ""
            row_output["translation_raw_response_social_labels_only"] = ""
            row_output["translation_status_social_labels_only"] = "failed"
            row_output["translation_error_social_labels_only"] = str(error)
            failure_count += 1
...
def print_translation_summary(result: SocialLabelsOnlyResult) -> None:
    """Print a concise run summary."""
    print("Social-label-guided translation complete")
...
__all__ = [
    ...
    "JOIN_KEYS",
    "SocialLabelsOnlyResult",
    "build_social_labels_only_prompt",
    "parse_social_labels_only_response",
    "print_translation_summary",
    "translate_social_labels_only_dataset",
]