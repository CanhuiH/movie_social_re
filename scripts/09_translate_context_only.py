...
"""Command-line entry point for context-only Mandarin translation."""
...
from src.translation.translate_context_only import (  # noqa: E402
    DEFAULT_API_KEY_ENV,
    DEFAULT_INPUT_PATH,
    DEFAULT_MODEL,
    DEFAULT_OUTPUT_PATH,
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate context-only Mandarin translations for graph-guided translation comparison."
    )
    parser.add_argument(
        "--input-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Output context-only translation CSV path.",
    )
    parser.add_argument(
        "--prompt-path",
        type=Path,
        default=DEFAULT_PROMPT_PATH,
        help="Context-only prompt template path.",
    )
    parser.add_argument(
        "--provider",
        type=str,
...
def main() -> None:
    ...
    if args.print_paths:
        print("Context-only translation paths:")
        print(f"  input : {input_path}")
        print(f"  prompt: {prompt_path}")
        print(f"  output: {output_path}")

    output_df = translate_context_only_dataset(
    ...
    result = summarize_context_only_output(
        output_df=output_df,
        output_path=output_path,
        provider=args.provider,
        model=args.model,
